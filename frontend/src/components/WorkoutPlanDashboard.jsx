...
function WorkoutDayCard({ day, onStartWorkout }) {
  return (
    <div className={`workout-day-card ${day.is_rest_day ? "is-rest" : ""}`}>
          ...
          {day.exercises.map((exercise) => (
            <li key={exercise.name} className="exercise-row">
              ...
              <div className="exercise-row__stats">
                <span>
                  {exercise.sets} × {exercise.is_time_based ? `${exercise.duration_seconds}s` : exercise.reps}
                </span>
      ...
      {!day.is_rest_day && (
        <button
          type="button"
          className="cta-button workout-day-card__start-button"
...
export default function WorkoutPlanDashboard({ workoutPlan, onStartWorkout }) {
  ...
  return (
    <section className="dashboard" aria-live="polite">
      ...
      <div className="workout-grid">
        {days.map((day) => (
          <WorkoutDayCard day={day} onStartWorkout={onStartWorkout} key={day.day_name} />
        ))}
      </div>
    </section>
  );

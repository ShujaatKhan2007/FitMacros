...
import WorkoutPlayer from "./components/coach/WorkoutPlayer.jsx";
import ChatWidget from "./components/ChatWidget.jsx";
import FoodPhotoAnalyzer from "./components/FoodPhotoAnalyzer.jsx";
...
export default function App() {
  ...
  // ---- Workout Coach Mode state ----
  // Holds the selected day's data while the guided workout session is
  // open, or null when Coach Mode is closed. Validation is implicit: the
  // "Start Workout" button that sets this only exists once a workout plan
  // has actually been generated (see WorkoutPlanDashboard below).
  const [activeCoachDay, setActiveCoachDay] = useState(null);
  ...
  return (
    <div className="app">
            ...
            <>
              ...
              {workoutStage === "done" && workoutPlan && !workoutLoading && (
                <WorkoutPlanDashboard
                  ...
                  onStartWorkout={(day) => setActiveCoachDay(day)}
                />
              )}
            </>
          )}
      ...
      <footer className="app__footer">
        ...
        <p className="app__footer-credit">Developed By Shujaat Khan</p>
      </footer>
      ...
      {activeCoachDay && (
        <WorkoutPlayer day={activeCoachDay} onExit={() => setActiveCoachDay(null)} />
      )}
    </div>
  );
}

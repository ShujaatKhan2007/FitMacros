...
# gemini-1.5-flash was retired by Google - all Gemini 1.0/1.5 models are
# shut down. gemini-2.5-flash-lite is the current stable, cost-efficient
# model recommended for lightweight tasks like this one. If Google
# retires this model too in the future, check
# https://ai.google.dev/gemini-api/docs/models for the current
# recommended replacement and update this one line.
GEMINI_API_URL = (
...
def get_ai_reply(message: str, context: dict = None) -> str:
    ...
    try:
        ...
        print(f"[ai_fallback] Gemini API error (status {status}): {body}")
        return None
    except Exception as error:
        print(f"[ai_fallback] Gemini API call failed: {type(error).__name__}: {error}")
        return None
